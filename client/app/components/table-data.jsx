...
import { Tooltip, Typography, IconButton } from "@material-tailwind/react";
...
function TableData(props) {
  ...
  const setSelectItems = () => {
    ...
    const rows = Array.from(checkedRows).map((ele) => ele.value); //Get Checkbox values that are checked.
      ...
      <table className="overflow-hidden w-full text-left">
        <thead>
          <tr>
            ...
            {Object.entries(TABLE_HEADS).map((head, index) => (
              <th
                key={index}
                className={`border-b border-blue-gray-100 bg-blue-gray-50 p-4 ${head[1]}`}
              >
                <Typography
                  variant="small"
                  color="blue-gray"
        ...
        <tbody className="max-w-full">
          ...
          {props.data.map((file, index) => {
            ...
            const isSelected = Object.keys(deleteContext.items).includes(
              filePathName
            );
            ...
            return (
              ...
              >
                {/* {console.log()} */}
                <td className="flex-col border-r border-b">
                  <Checkbox
                ...
                >
                  <TableNameItem fileInfo={file} {...tableNameItemProps} />
                </td>
                <td className="sm:table-cell hidden border-b">
                ...
                <td className=" border-b">
                  <div className="flex gap-3 ">
                    ...
                    {file.type !== 1 && <DownloadItem fileName={file.name} />}
                  </div>
                </td>
              </tr>
            );
